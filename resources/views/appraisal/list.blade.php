...
@section('content')
@section('page-title', 'Appraisal Master')
@section('page-heading', 'Appraisal List')
                ...
                <div id="kt_customers_table_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer"><div class="table-responsive">
                    ...
                    <thead>
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                            ...
                            <th class="min-w-17.20px sorting"  aria-controls="kt_customers_table" aria-label="Process: activate to sort column ascending" style="width: 17.20px;">Process</th>
                           <th class="min-w-15px sorting">Sub Process</th>
						   <th class="min-w-15px sorting">Employee ID</th>
						   <th class="min-w-15px sorting">Created By</th>
                           <th  class="min-w-15px sorting">Created On</th>
                    ...
                    <tr class="even">
                            <td>{{ $i }}</td>
                            <td class="location">{{ $list->loc }}</td>
                            <td class="cm_id" data="{{ $list->cm_id }}">{{ $list->process }}</td>
							<td class="sub_process">{{ $list->sub_process }}</td>
                            <td class="client_time_ttl">{{ $list->EmpID }}</td>
							<td class="client_time_min">{{ $list->createdby }}</td>
...
<div class="modal fade" id="edit-appraisal-slab" tabindex="-1" aria-hidden="true" role="dialog">
    <!--begin::Modal dialog-->
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <!--begin::Modal content-->
        <div class="modal-content">
            <form class="form fv-plugins-bootstrap5 fv-plugins-framework" action="{{ url('save-appraisal-slab') }}"  method="post" id="edit_appraisal_slab_form" data-kt-redirect="">
                @csrf
                   <div class="modal-header" id="kt_modal_edit_downtime_header">
                          <h2 class="fw-bold">Apraisal Details</h2>
                ...
                <div class="modal-body">
                        <div class="row fv-plugins-icon-container">
                            ...
                            <div class="col-md-4 col-sm-4 col-lg-4 fv-row">
                                ...
                                <input type="number" id="employee_id" name="employee_id" placeholder="Enter Employee ID !" class="form-control">
                                <div class="fv-plugins-message-container fv-plugins-message-container--enabled invalid-feedback"></div>
                            </div>
                        </div>
                     </div>
...
<script>
    $(function(){
        ...
        $('#edit-salary-slab_form').on('submit',function(event){
           ...
           $('.edit-apprasal-master').on('click',function(){
