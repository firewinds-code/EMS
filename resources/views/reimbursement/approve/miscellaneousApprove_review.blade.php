...
<div id="forResponse">
            ...
            <div class="table-responsive">
               ...
               <table id="example" class="table align-middle table-row-dashed fs-6 gy-5">
                  ...
                  <tbody class="fw-semibold text-gray-600">
                     @if ($miscData->isNotEmpty())
                     @foreach ($miscData as $employeeData)
                     <tr>
                        ...
                        <td id="MempId">{{ $employeeData->EmployeeID }}</td>
                        <td id="MempName">{{ $employeeData->empName }}</td>
                        <td id="Mdate">{{ $employeeData->date }}</td>
                        <td id="Mammount">{{ $employeeData->amount }}</td>
                        <td id="MStatus">{{ $employeeData->req_status }}</td>
                        <td id="MRecipt">{{ $employeeData->receipt_no }}</td>
                        <td id="MDownloadRecipt">
                           <a href="{{ asset('reimbursement/ExpenseMiscellaneous/' . $employeeData->receipt_image) }}" download>
                              <i class="fas fa-download text-success" title="Download File"></i>
                           </a>
                        </td>
                        <td id="MRemakrs">{{ $employeeData->remarks }}</td>
                        @if ($type === 'approve')
                        ...
                        <td hidden id="miscID">{{ $employeeData->id }}</td>
         ...
         <div class="modal-content">
            ...
            <div class="modal-body ">
               <form id="edit_hotel" class="form" method="POST" enctype="multipart/form-data">
                  @csrf
                  <input type="text" class="form-control" readonly maxlength="10" hidden placeholder="Receipt No" id="HotelReceiptFile" name="HotelReceiptFile" />
                  <input type="text" class="form-control" readonly maxlength="10" hidden placeholder="Receipt No" id="miscID_" name="miscID_" />
                  <div class="row g-9 mb-8">
                     <div class="col-md-4 fv-row">
                        ...
                        <input type="text" class="form-control" placeholder="Pick date rage" id="miscDate" name="miscDate" readonly />
                     </div>
                     <div class="col-md-4 fv-row">
                        <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                           <span>Amount</span>
                        ...
                        <input type="text" readonly class="form-control" placeholder="Enter Ammount" id="miscAmount" name="miscAmount" />
                     </div>
                     <div class="col-md-4 fv-row">
                        <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                        ...
                        <input type="text" class="form-control" readonly maxlength="10" placeholder="Receipt No" id="miscReceiptNo" name="miscReceiptNo" />
                     </div>
                     ...
                     <div class="col-md-6 fv-row">
                        ...
                        <textarea class="form-control" rows="3" name="miscRemakrs" readonly id="miscRemakrs" placeholder="Type Comment Details"></textarea>
                     </div>
                     @if ($type === 'approve')
                     <div class="col-md-6 fv-row">
                        <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                        ...
                        <input type="text" class="form-control" readonly maxlength="10"  id="miscStatusReviewer" name="miscStatusReviewer" />
                     </div>
                     <div class="col-md-6 fv-row">
                        <label class="fs-6 fw-semibold mb-2">Reviewer Remarks</label>
                        <textarea class="form-control" rows="3" name="miscRemakrsReviewer" readonly id="miscRemakrsReviewer" placeholder="Type Comment Details"></textarea>
                     </div>
                     ...
                     <div class="d-flex flex-column mb-8">
                        ...
                        <textarea class="form-control" rows="3" name="miscCommentApprover" placeholder="Type Comment Details"></textarea>
                        <div class="invalid-feedback">
                     ...
                     <div class="d-flex flex-column mb-8">
                        <label class="required">Reviewer Comment</label>
                        <textarea class="form-control" rows="3" name="miscCommentReviewer" placeholder="Type Comment Details"></textarea>
                        <div class="invalid-feedback">
   ...
   <script>
      ...
      document.getElementById("edit_hotel").addEventListener("submit", function(event) {
         ...
         var commentTextarea = reqType === "review" ? document.querySelector("[name='miscCommentReviewer']") :
            document.querySelector("[name='miscCommentApprover']");
         ...
         if (isValid) {
            ...
            $.ajax({
               url: "{{ route('createReiseMiscellaneous') }}",
               method: 'POST',
               data: formData,
               processData: false,
               contentType: false,
      ...
      function editfoodModal(el) {
         ...
         var MiscID = tr.find('#miscID').text();
         var Date = tr.find('#Mdate').text();
         var Amount = tr.find('#Mammount').text();
         var MRemakrs = tr.find('#MRemakrs').text();
         var mobileReceiptNo = tr.find('#MRecipt').text();
         var mobileReceipt_File = tr.find('#receipt_File').text();
         ...
         $('#miscStatusReviewer').val(ReviewerStatus);
         $('#miscRemakrsReviewer').val(ReviewerRemakrs);

         $('#miscDate').val(Date);
         $('#miscAmount').val(Amount);
         $('#miscRemakrs').val(MRemakrs);
         $('#miscReceiptNo').val(mobileReceiptNo);
         $('#miscID_').val(MiscID);
         var downloadLink = $('#downloadLink');
         var downloadUrl = `{{ asset('reimbursement/ExpenseMiscellaneous/') }}/` + mobileReceipt_File;
